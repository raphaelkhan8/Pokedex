...
const getUserPokemon = userId => UsersPokemon.findAll({
  where: {
    userId,
  },
  include: [Pokemon],
...
const searchPokemon = query => axios.get(`https://pokeapi.co/api/v2/pokemon/${query}`)
  .then((response) => {
...
const addPokemonToCollection = (userId, pokemon) => {
  ...
  }).then(([pokemon]) => Users.findOne({
    where: {
      id: userId,
    },
  })
    .then(userInfo => UsersPokemon.create({ userId: userInfo.id, pokeId: pokemon.id })
      .then(() => {
